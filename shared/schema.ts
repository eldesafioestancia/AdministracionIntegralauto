...
// Reproductive Management - Natural Service (Monta Natural)
export const naturalServices = pgTable("natural_services", {
  id: serial("id").primaryKey(),
  animalId: integer("animal_id").notNull(),
  animalCartagena: text("animal_cartagena"),
  bullEntryDate: timestamp("bull_entry_date").notNull(),
  bullExitDate: timestamp("bull_exit_date"),
  bullId: integer("bull_id").notNull(),
  bullCartagena: text("bull_cartagena"),
  pregnancyCheckDate: timestamp("pregnancy_check_date"),
  pregnancyResult: text("pregnancy_result"), // prenada, vacia, duda
...
// Base natural service schema
const baseNaturalServiceSchema = createInsertSchema(naturalServices).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// Extended schema with date transformations
export const insertNaturalServiceSchema = baseNaturalServiceSchema.extend({
  bullEntryDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ),
  bullExitDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ).optional().nullable(),
  pregnancyCheckDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ).optional().nullable(),
...
// Reproductive Management - Artificial Insemination (Inseminación Artificial)
export const artificialServices = pgTable("artificial_services", {
  id: serial("id").primaryKey(),
  animalId: integer("animal_id").notNull(),
  animalCartagena: text("animal_cartagena"),
  devicePlacementDate: timestamp("device_placement_date").notNull(),
  deviceRemovalDate: timestamp("device_removal_date").notNull(),
  inseminationDate: timestamp("insemination_date").notNull(),
  bullId: integer("bull_id").notNull(),
  bullIdentification: text("bull_identification"),
  pregnancyCheckDate: timestamp("pregnancy_check_date"),
  pregnancyResult: text("pregnancy_result"), // prenada, vacia, duda
  dueDate: timestamp("due_date"),
  observations: text("observations"),
...
// Base artificial service schema
const baseArtificialServiceSchema = createInsertSchema(artificialServices).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// Extended schema with date transformations
export const insertArtificialServiceSchema = baseArtificialServiceSchema.extend({
  devicePlacementDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ),
  deviceRemovalDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ),
  inseminationDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
  ),
  pregnancyCheckDate: z.string().or(z.date()).transform(val => 
    typeof val === 'string' ? new Date(val) : val
...
// Add export types for reproductive services
export type NaturalService = typeof naturalServices.$inferSelect;
export type InsertNaturalService = z.infer<typeof insertNaturalServiceSchema>;

export type ArtificialService = typeof artificialServices.$inferSelect;
export type InsertArtificialService = z.infer<typeof insertArtificialServiceSchema>;
