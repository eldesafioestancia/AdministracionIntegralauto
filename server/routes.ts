...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // --- Rutas para manejo reproductivo ---
  
  // Servicios naturales (monta natural)
  app.get("/api/reproductive/natural", async (req: Request, res: Response) => {
    try {
      const services = await storage.getNaturalServices();
      res.json(services);
    } catch (error) {
      console.error("Error fetching natural services:", error);
      res.status(500).json({ message: "Error fetching natural services" });
    }
  });
  
  app.post("/api/reproductive/natural", async (req: Request, res: Response) => {
    try {
      const service = await storage.createNaturalService(req.body);
      res.status(201).json(service);
    } catch (error) {
      console.error("Error creating natural service:", error);
      res.status(500).json({ message: "Error creating natural service" });
    }
  });
  
  app.put("/api/reproductive/natural/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const updated = await storage.updateNaturalService(id, req.body);
      
      if (!updated) {
        return res.status(404).json({ message: "Natural service not found" });
      }
      
      res.json(updated);
    } catch (error) {
      console.error("Error updating natural service:", error);
      res.status(500).json({ message: "Error updating natural service" });
    }
  });
  
  app.delete("/api/reproductive/natural/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const deleted = await storage.deleteNaturalService(id);
      ...
      console.error("Error deleting natural service:", error);
      res.status(500).json({ message: "Error deleting natural service" });
    }
  });
  
  // Inseminación artificial
  app.get("/api/reproductive/artificial", async (req: Request, res: Response) => {
    try {
      const services = await storage.getArtificialServices();
      res.json(services);
    } catch (error) {
      console.error("Error fetching artificial services:", error);
      res.status(500).json({ message: "Error fetching artificial services" });
    }
  });
  
  app.post("/api/reproductive/artificial", async (req: Request, res: Response) => {
    try {
      const service = await storage.createArtificialService(req.body);
      res.status(201).json(service);
    } catch (error) {
      console.error("Error creating artificial service:", error);
      res.status(500).json({ message: "Error creating artificial service" });
    }
  });
  
  app.put("/api/reproductive/artificial/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const updated = await storage.updateArtificialService(id, req.body);
      
      if (!updated) {
        return res.status(404).json({ message: "Artificial service not found" });
      }
      
      res.json(updated);
    } catch (error) {
      console.error("Error updating artificial service:", error);
      res.status(500).json({ message: "Error updating artificial service" });
    }
  });
  
  app.delete("/api/reproductive/artificial/:id", async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const deleted = await storage.deleteArtificialService(id);
      ...
      console.error("Error deleting artificial service:", error);
      res.status(500).json({ message: "Error deleting artificial service" });
    }
  });
